...
class CurrencyTypeAdapter  (private val mModelList: List<CurrencyTypeModel.Data>?, val context: Context) :
    RecyclerView.Adapter<CurrencyTypeAdapter.MyViewHolder>() {
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): CurrencyTypeAdapter.MyViewHolder {
        val view: View =
            LayoutInflater.from(parent.context).inflate(R.layout.payment_item_cell, parent, false)
        return MyViewHolder(view)
    }
    override fun onBindViewHolder(holder: CurrencyTypeAdapter.MyViewHolder, position: Int) {
        val model = mModelList!![position]
        holder.titlec.setText(model.currenciesCode)
        holder.textBack.setText(model.currenciesName)
        if (model.isSelected){
...
//            if (model.isSelected) context.resources.getColor(R.color.white) else context.resources.getColor(R.color.appMainColor))
        holder.image_gradient.setOnClickListener {
            ...
            common.userCurrencyType.add(model.id!!)
            if (model.isSelected){
                holder.image_gradient.setBackgroundColor(context.resources.getColor(R.color.appMainColor))
                holder.titlec.setTextColor(context.resources.getColor(R.color.white))
                holder.textBack.setTextColor(context.resources.getColor(R.color.card2))
